...
const FloatingCallButton = () => {
  ...
  const base =
    "fixed left-0 top-1/2 -translate-y-1/2 z-50 md:hidden bg-accent text-accent-foreground rounded-r-full shadow-lg hover:bg-accent/90 transition-all duration-500 flex items-center";

  const visibility = isVisible ? "opacity-100" : "opacity-0 pointer-events-none";

  const expandedClasses = "h-12 w-auto pl-3 pr-4 gap-2 justify-start";
  const collapsedClasses = "h-12 w-12 px-0 gap-0 justify-center translate-x-[2px]";
